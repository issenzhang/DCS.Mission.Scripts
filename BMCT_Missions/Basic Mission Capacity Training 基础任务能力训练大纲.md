...
## **训练3 BE&INTERCEPT 靶眼参考和战术拦截(昼间/夜间)**
...
1. 热机启动, 小队内部自行决定起飞方式.
2. 抵达wypt.2并按照任务要求建立wypt.2-wypt.3巡逻航线.
3. 根据空情通报, 正确地完成空中拦截任务
   1. 要求在5min内完成拦截任务.
   2. 要求<u>稳定伴飞 ≥ 60 sec</u>
      1. 稳定伴飞要求如下图
      2. <u>与目标的高度差 ≤ 500 ft</u>![image-20230920084905335](Basic Mission Capacity Training 基础任务能力训练大纲.assets/image-20230920084905335.png)
   3. 如有训练管制, 向其通报拦截飞机的机型
   4. 提示完成拦截任务后, 再次返回wypt.2重新建立wypt.2-3巡逻航线
4. 可以根据需要, 前往加油机处补充燃油
...
## 训练#4 TFM&ACM(BASIC) 战术编队与ACM(基础)
...
## **训练#5 TFM&ACM(TURNNING) 战术编队与ACM(转弯)**

> 本训练是虚拟笕桥航校基础班教官TDK的战术转弯&编队警戒训练的复刻(已经得到了TDK大佬的许可)，在此由衷感谢。
...
### 2.  训练流程
 ...
 3. 机组起飞后, 飞往wypt.1建立等待航线

 4. 按照管制要求, 飞向训练空域, 并在<u>途中完成FENCE IN检查和展开战斗横队队形</u>.

     1. <u>训练空域</u>是由导航点2~5, 6~9, 10~13 围成的训练空域

     2. 训练过程中要求不能离开训练空域超过90秒, 否则判定训练失败(会有消息提示)

 5. 在空域内飞行期间, 维持<u>KCAS 400/ 15000ft</u>, <u>注意相互瞭望, 观察可能出现的敌机</u>
 ...
 6. 消灭<u>三波次</u>敌机后, 双机向wypt.1转向退出空域

 7. 抵达wypt.1后, 可以选择继续着陆, 或直接返回观众席, 或根据训练管制指令操作
...
- 全程正确使用无线电通联规范
- 在wypt.1-训练空域飞行期间, 正确地完成了战斗横队队形展开
- 在训练空域内飞行期间, 正确地完成了战斗横队队形以及战术转弯
- 在所有波次训练中均击落敌机而己方无损失
