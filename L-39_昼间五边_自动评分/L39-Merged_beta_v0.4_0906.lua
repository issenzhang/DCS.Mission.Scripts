-- 使用方法:
-- 请按照《虚拟笕桥航校基础班昼间五边考试规范220930》的流程规范, 完成完整的L-39昼间五边起降训练
-- Tips: 也可从Bravo直接上跑道(但需要进跑道前短停),但联机练习时候不要这么做哦

-- 重要说明:
-- 本地图基于《虚拟笕桥航校基础班昼间五边考试规范220930》制作
-- 本地图仅供娱乐使用
-- 本地图完全不能代替日常训练地图
-- 本地图不能作为模拟考核的参考

-- 由于作者对考核规范的理解不充分，本地图无法涵盖考核的范围

-- 基于作者的菜鸡水平，包括但不限于以下扣分项目无法体现：
-- - 无线电通联的不规范使用
-- - 滑行过程中的偶然超速
-- - 起飞阶段的起落架状态和襟翼档位切换
-- - 俯仰配平的使用不熟练
...
-- 此外，包括但不限于以下的扣分项目，可能存在本地图打分明显高于教官评分的情况：
-- - 三边参数保持能力
-- - 起落架释放过晚
-- - 进近阶段,过晚地对准跑道
-- - 近/远台高度判断

-- 感谢娃娃,神棍,兔子同学对地图的测试和意见反馈

-- by ISSEN / issen.gamming@outlook.com

-- todo：改为线上多人版本
-- todo：增加对起落架/襟翼的判断（通过动画）
-- todo：远期：变更为使用MOOSE:FSM框架
...
function L39:CheckPattern_Rolling()
    ...
    if speed >= SPEED_ROLLING_PITCH then
        ...
        if pitch >= 20 then
            self:AddPenaltyByDetail("rolling_pitch", 2, "起飞擦尾(滑跑迎角≥20°)", pitch)
        end
    end
    ...
    if self:IsInZone("z-rolling") then
        local heading = self._unit:GetHeading(false)
        local heading_delta = math.abs(87 - heading)
        if heading_delta > 5 then
            self:AddPenaltyByDetail("rolling_angle", 2, "滑跑偏航≥5°", heading)
        elseif heading_delta > 3 then
            self:AddPenaltyByDetail("rolling_angle", 1, "滑跑偏航≥3°", heading)
        end

        if self:IsNotInZone("z-rolling-center") then
            self:AddPenaltyByDetail("rolling_not_center", 5, "滑跑未压中线")
        end
    end
...
function L39:CheckPattern_Upwind()
    ...
    if self:InAir() == false then
        self:AddPenaltyByDetail("takeoff_touch_ground", 5, "起飞反复接地")
    end
    return self
end
...
function L39:CheckPattern_Crosswind()
    ...
    elseif roll >= ROLL_MAX_1 then
        self:AddPenaltyByDetail("upwind_roll", 1, "一转三坡度≥30°", roll)
    end
    return self
end
...
function L39:CheckPattern_Base()
    ...
    if ias <= 200 then
        self:AddPenaltyByDetail("base_lowspeed", 3, "二转空速≤200Km/h", ias)
    end
...
function L39:CheckPattern_Final()
    ...
    elseif ias <= 280 then
        self:AddPenaltyByDetail("final_overspeed", 3, "进近空速≤200Km/h", ias)
    end
...
-- local timer_mission = TIMER:New(
--     function()
--     -- add l39 train objects
--     local clients = set_clients:GetSetObjects()
--     for _, v in ipairs(clients) do
--         if v:IsInZone(ZONE:New("z-init")) == true and set_inTraining:IsNotInSet(v) then
--             init_test(v)
--         end
--     end

--     --remove l39 train finished objects
--     for i,l39 in ipairs(tablel39) do
--     if l39.is_trainning ==false then 
--         set_inTraining:RemoveClientsByName(l39._client.ClientName)    
--         table.remove(tablel39,i)
--         break
--     end
-- end

--     for _,c in ipairs(set_inTraining:GetSetObjects()) do
...
--     if flagRemove then
--         for i,l39 in ipairs(tablel39) do
--             if l39._client.ClientName == c.ClientName then
--                 set_inTraining:RemoveClientsByName(l39._client.ClientName)
--                 table.remove(tablel39,i)
--             end
--         end
...
local timer_mission = TIMER:New(
    function()
        ...
        -- Create a list of indices to remove from tablel39
        local indicesToRemove = {}

        -- remove l39 train finished objects
        for i, l39 in ipairs(tablel39) do
            if l39.is_training == false then
                set_inTraining:RemoveClientsByName(l39._client.ClientName)
                table.insert(indicesToRemove, i)
            end
        end

        -- Remove elements from tablel39 in reverse order to avoid index issues
        for i = #indicesToRemove, 1, -1 do
            table.remove(tablel39, indicesToRemove[i])
        end

        -- Create a list of clients to remove from set_inTraining
        local clientsToRemove = {}

        for _, c in ipairs(set_inTraining:GetSetObjects()) do
            local flagRemove = false
            if c == nil or c:IsActive() == false then
                flagRemove = true
            end

            if flagRemove then
                table.insert(clientsToRemove, c)
            end
        end

        -- Remove clients from set_inTraining
        for _, client in ipairs(clientsToRemove) do
            set_inTraining:RemoveClientsByName(client.ClientName)
            for i = #tablel39, 1, -1 do
                if  #tablel39[i]._client.ClientName == client.ClientName then
                    table.remove(tablel39, i)
                end
            end
        end
