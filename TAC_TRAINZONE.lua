-- TAC_TRAINZONE 类是一个用于控制飞行训练区域的Lua脚本类。
...
TAC_TRAINZONE = {
    ClassName = "TAC_TRAINZONE"
}

--- Parameters ---

TAC_TRAINZONE.IsDebugMessage = true -- 是否显示调试消息
TAC_TRAINZONE.TimerStep = 5 -- 计时器步长

-- NOTES: Default Spawn settings is for tfm-acm train
-- 注意: 默认的生成设置适用于tfm-acm模式的训练

TAC_TRAINZONE.IsSpawnEmeny = true -- 是否生成敌人
TAC_TRAINZONE.IsEndlessMode = false -- 是否无限模式
TAC_TRAINZONE.MaxTrainWavesFinished = 3 -- 最大训练波数

TAC_TRAINZONE.TableEnemyTemplate = {"enemy-f16-highvis", "enemy-f16-lowvis", "enemy-f16-lowvis-15",
                                    "enemy-f16-lowvis-30"} -- 敌人模板列表
TAC_TRAINZONE.TableSpawnAlt = {1000, 15000, 30000} -- 生成敌人的高度（米） 
TAC_TRAINZONE.TableSpawnBRA = {0, 45, 90, 120, 180} -- 生成敌人的方向（度）

TAC_TRAINZONE.IsSmartSpawn = true -- 是否智能生成(根据迎击角度改变生成距离)
TAC_TRAINZONE.SpawnDistance_At180 = 8 -- 冷启动时的生成距离
TAC_TRAINZONE.SpawnDistance_At0 = 20 -- 热启动时的生成距离
TAC_TRAINZONE.SpawnDistance_K = 0 -- 生成距离系数(标定无用,函数内重新计算)
TAC_TRAINZONE.SpawnDelayMin = 60 -- 生成敌人的最小延迟时间（秒）
TAC_TRAINZONE.SpawnDelayMax = 240 -- 生成敌人的最大延迟时间（秒）

TAC_TRAINZONE.EnemyDestoryDelay = 30 -- 敌人销毁延迟时间（秒）
TAC_TRAINZONE.EnemyEmissionOpenDelay = 30 -- 敌人开启雷达延迟时间（秒）

TAC_TRAINZONE.OutboundMaxDuration = 90 -- 出界最大持续时间（秒）

---  Parameters End---

--- values ---

TAC_TRAINZONE.GroupTrain = nil
TAC_TRAINZONE.GroupEnemy = nil

TAC_TRAINZONE.TimerTraining = nil
TAC_TRAINZONE.TrainWavesFinished = nil

TAC_TRAINZONE.SpawnTicker = 0
TAC_TRAINZONE.SpawnTickerTrigger = 0

TAC_TRAINZONE.OutboundTicker = 0

--- values end ---

--- functions ---
function TAC_TRAINZONE:ShowMessage(Message, Duration, IsClean, IsToAll, ToGroup)
...
function TAC_TRAINZONE:New(ZoneName)
...
function TAC_TRAINZONE:SpawnEnemy()
    -- ref
...
function TAC_TRAINZONE:OnEnterIdle(From, Event, To)
...
function TAC_TRAINZONE:OnEnterTraining(From, Event, To)
...
function TAC_TRAINZONE:OnBeforeEnemySpawn(From, Event, To)
...
function TAC_TRAINZONE:OnBeforeAbortOrFinish(From, Event, To)
...
function TAC_TRAINZONE:OnEventHit(EventData)
...
function TAC_TRAINZONE:AllKilled()
    self:ShowMessage("GoodKill~ GoodKill~")
    self.TrainWavesFinished = self.TrainWavesFinished + 1
    if self.IsEndlessMode or self.TrainWavesFinished < self.MaxTrainWavesFinished then
        -- continue spawn
...
function TAC_TRAINZONE:OnEventCrash(EventData)
