...
local function reportGroupPosVsPoint(_group, _point, _point_name)
    if _group ~= nil and _point ~= nil then
        ...
        return "目标相对" .. _point_name .. ":\n" ..
            " - 相对方位：" .. string.format("%0.1f", UTILS.Round(_point:HeadingTo(pos), 1)) .. " °(真航向) " ..
            " - 相对距离：" .. string.format("%0.2f", UTILS.Round(dis / 1000, 2)) .. " nm " ..
            " - 高度：" .. string.format("%0.2f", UTILS.Round(tgt_height, 2)) .. "m"
    end
end

-- 返回群组相对于坐标点的位置
local function reportPointPosVsRegPoint(_point_toReport, _point, _point_name)
    local pos = _point_toReport

    local ang = _point:HeadingTo(pos)
    local dis = pos:Get2DDistance(_point)
    local tgt_height = pos:GetLandHeight()
    --      return pos:ToStringFromRP(_point, "TGT1")
    --          .. " m, 海拔高度: " .. tgt_height .. " m"
    --          .. " 精确距离：" .. math.ceil(dis) .. " m"

    return "目标相对" .. _point_name .. ":\n" ..
        " - 相对方位：" .. string.format("%0.1f", UTILS.Round(ang, 1)) .. " °(真航向)\n" ..

        --贴近考核,改相对距离的通报方式为海里
        " - 相对距离：" .. string.format("%0.2f", UTILS.Round(dis / 1852, 2)) .. " nm\n" ..
        " - 高度：" .. string.format("%0.2f", UTILS.Round(tgt_height, 2)) .. "m"
end
...
local function spawnTgt1()
    ...
    MESSAGE:New("目标信息通报:", 60):ToAll()

    for i, v in ipairs(rand_zones) do
        local zoneName = "z-tgt1-" .. 1 .. "-" .. v
        local spawnZone = ZONE:New(zoneName)
        local temp_rand = GenerateRandomUniqueNumbers(1, 12)[1]
        local pos = spawnZone:GetCoordinate()

        if temp_rand <= 7 then
            local spawnTgt2 = SPAWN
                :NewWithAlias("tgp-" .. temp_rand, "tgt1-" .. 1 .. "#" .. TGT1_NUM)
                :InitLimit(2, 2)
                :InitHeading(0, 359)
            local g = spawnTgt2:SpawnInZone(spawnZone)
        else
            local spawnTgt1_Static = SPAWNSTATIC:NewFromStatic("tgp-" .. temp_rand)
            spawnTgt1_Static:SpawnFromZone(spawnZone, TGT1_NUM)
        end

        local g_msg =
            "目标#" .. i .. ":\n"
            .. "类型:" .. TGT1_typeWord[temp_rand] .. "\n"
            .. reportPointPosVsRegPoint(pos, ZONE:New(TGT1_ZONE_NAME):GetCoordinate(), "TGT1")
        env.info("#" .. i .. "-" .. g_msg)
        MESSAGE:New(g_msg, 120):ToAll()
...
--#region 低空飞行段
-- 低空段参数
local LOWFILGHT_HEIGHT_THRESHOLD = 121.90 --低空飞行段的限高(400ft)
...
local function reportPositionRandom(group)
    local str = {}
    str[1] = group:GetCoordinate():ToStringLLDDM() .. "|" .. math.ceil(group:GetCoordinate():GetLandHeight()) .. " m"
    str[2] = group:GetCoordinate():ToStringMGRS() .. "|" .. math.ceil(group:GetCoordinate():GetLandHeight()) .. " m"
    return str[GenerateRandomUniqueNumbers(1, 2)[1]]
end
...
local function spawnTgt3()
    ...
    MESSAGE:New("2组迫击炮小组坐标:", 90):ToBlue()

    local tgt3_1_pos = "#1 " .. reportPositionRandom(tgt3_1)
    local tgt3_2_pos = "#2 " .. reportPositionRandom(tgt3_2)
...
local MenuTgt1Spawn_1 = MENU_COALITION_COMMAND:New(coalition.side.BLUE, "激活吊舱检索目标", MenuTgt1,
    spawnTgt1)
